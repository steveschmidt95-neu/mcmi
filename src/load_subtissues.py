...
class ROI_Data():
    ...
    def __init__(self, msi_data, sliced_roi_positions, tuple_positions, subtissue_labels):
        ...
        self.labels[:, :] = -1 # initialize all to -1 to keep track of missing subtissue labels
        ...
        for line in range(0, sliced_roi_positions.shape[0]):
            ...
            for position_row in range(0, tuple_positions.shape[0]):
                ...
                if checkx == x and checky == y:
                    ...
                    self.labels[line, 0] = sliced_roi_positions[line, 4]
                    
            for sub_line in range(0, subtissue_labels.shape[0]):
                x_sub = int(subtissue_labels[sub_line, 0])
                y_sub = int(subtissue_labels[sub_line, 1])
                 
               
                if x_sub == x and y_sub == y:
                    self.labels[line, 1] = subtissue_labels[sub_line, 4]
...
class MSIData():
    ...
    def __init__(self, data_name = 'msi.h5', tab_name = 'final_annotation_complete_cores_2.tabular', sub_tab_name = 'subtissue_labels.tabular'):
        ...
        for line in open(self.tab_path):
            ...
            line_count+=1
        self.position_data_complete = position_data
        ...
        for line in open(self.sub_tab_path):
            ...
            position_data[line_count-1, 1] = int(line_data[1]) # Y
            position_data[line_count-1, 2] = int(line_data[2][-1]) # TMA?
            position_data[line_count-1, 3] = int(line_data[5][3:]) # ROINUM
        ...
        print('Number of ROIs', np.max(self.position_data_complete[:, 3]))
        ...
        for roi in range(1, num_rois):
            ...
            sliced_positions = self.position_data_complete[np.where(self.position_data_complete[:,3] == roi)]
            ...
            if roi < 24:
                ...
                roi_data = ROI_Data(self.msispec, sliced_positions, self.position, self.position_data_complete)
                roi_data.save_h5()    
        ...
        if val == 1:
            for key in list(self.val_files.keys()):
                if 'Label' in key:
                    ...
                    print('Missing Subtissue Labels: ', key, print(len(np.where(array[:,1]==-1)[0])))
                    occurances = collections.Counter(array[:,1])
